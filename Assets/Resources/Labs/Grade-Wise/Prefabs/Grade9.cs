...
public class Grade9 : MonoBehaviour
{
    ...
    public void gotoClass()
    {
        ...
        if (accesibleClasses.Contains("9")) {                                       //CHANGE "->  <-"

            //Call Respective subjects in Chapters-UI 
            mainPlayer.GetComponent<LabRoomManager>()._9ClassChapterLab();                    //CHANGE METHOD-NAME()

        }
        else if (responseText.Contains("all"))
        {
            //Call Respective subjects in Chapters-UI 
            mainPlayer.GetComponent<LabRoomManager>()._9ClassChapterLab();
        }
