...
class ExportTerrain : EditorWindow
{
    ...
    void DirectorySanityCheck()
    {
        ...
        Debug.Log("Existing directories count: " + directories);

        // If no subdirectory exists, create the first "0" folder
        if (directories == 0)
        {
            Directory.CreateDirectory(Path.Combine(pathToSaveTexture, "0"));
            pathToSaveTexture = Path.Combine(pathToSaveTexture, "0");
        }
        else
        {
            ...
            {
                string newDirectoryPath = Path.Combine(pathToSaveTexture, i.ToString());
                if (!Directory.Exists(newDirectoryPath))
                {
                    Directory.CreateDirectory(newDirectoryPath);
                    pathToSaveTexture = newDirectoryPath;
                    break; // Stop after creating the first available folder
                }
            }
        }

        Debug.Log("Final path to save texture: " + pathToSaveTexture);
    }
    ...
    int GetSizeMultiplier()
    {
        switch (textureResolution)
        {
            ...
            case SizeOfTexture.Texture2048x2048:
                return 4;
            case SizeOfTexture.Texture4096x4096:
                return 8;
        }
