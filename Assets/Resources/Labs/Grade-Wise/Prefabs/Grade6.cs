...
public class Grade6 : MonoBehaviour
{
    ...
    public void gotoClass()
    {
        ...
        if (accesibleClasses.Contains("6")) {                                          //CHANGE "->  <-"

            //Call Respective subjects in Chapters-UI 
            mainPlayer.GetComponent<LabRoomManager>().SixClassChapterLab();                    //CHANGE METHOD-NAME()

        }
        else if (responseText.Contains("all") )
        {
            //Call Respective subjects in Chapters-UI 
            mainPlayer.GetComponent<LabRoomManager>().SixClassChapterLab();
        }
        else
        {
            Debug.Log(StaticVariables.grade_class_name); 
