...
public class Server : MonoBehaviour
{
    ...
    private float window_left = Screen.width/ 2f - Screen.width/ 4f; 
    private float window_top = Screen.height/ 2f - Screen.height/ 4f;  
    private float window_width = Screen.width/2f; 
    private float window_height = Screen.height/2f;
    ...
    string rootURL = "https://juztag.com/test/Unity/"; //Path where php files are located
    ...
    {
        var newText = area ? GUILayout.TextArea(text) : GUILayout.TextField(text);
        if (string.IsNullOrEmpty(text.Trim()))
        {
            ...
            GUI.TextField(position, placeholder);
            GUI.color = guiColor;
        }
        return newText;
    }
    ...
    void OnGUI()
    {
        if (!isLoggedIn)
        {
            ...
            {
                //GUI.color = Color.red;
               // GUI.DrawTexture(new Rect(window_left, window_top, window_width, window_height), aTexture, ScaleMode.StretchToFill, true, 10.0F);
                // GUI.Window(0, new Rect(Screen.width / 2 - 125, Screen.height / 2 - 115, 250, 230), LoginWindow, "Login");
                GUI.Window(0, new Rect(window_left, window_top , window_width, window_height), LoginWindow, "Login");
            }
    ...
    void LoginWindow(int index)
    {
        ...
        loginEmail = GUILayout.TextField(loginEmail == string.Empty ? "enter name" : loginEmail);
        ...
        if (GUILayout.Button("Register", GUILayout.Width(95)))
        {
            ResetValues();
